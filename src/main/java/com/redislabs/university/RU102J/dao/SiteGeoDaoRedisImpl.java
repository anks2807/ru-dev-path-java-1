...
public class SiteGeoDaoRedisImpl implements SiteGeoDao {
    ...
    public Set<Site> findAll() {
        try (Jedis jedis = jedisPool.getResource()) {
            ...
            Map<String, Response<Map<String, String>>> siteResponsesMap = new HashMap<>();
            for (String key : keys) {
                siteResponsesMap.put(key, pipeline.hgetAll(key));
            }
            pipeline.sync();
            for (Map.Entry<String, Response<Map<String, String>>> entry : siteResponsesMap.entrySet())  {
                sites.add(new Site(entry.getValue().get()));
            }
    ...
    private Set<Site> findSitesByGeoWithCapacity(GeoQuery query) {
        ...
        GeoUnit radiusUnit = query.getRadiusUnit();
         ...
         try (Jedis jedis = jedisPool.getResource()) {
             ...
             List<GeoRadiusResponse> radiusResponses = jedis.georadius(RedisSchema.getSiteGeoKey(), coord.lat, coord.lng, radius, radiusUnit);
             // END Challenge #5
